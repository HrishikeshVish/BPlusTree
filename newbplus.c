...
void loadfromfile(node * p,int value, int index){
	...
	if(fp!=NULL){
		for(i = 0; i<root->num_keys; i++){
			...
			sprintf(n2, "%d",t2);
			printf("n2 = %s\n", n2);
			t = load(root, (node*)root->pointers[i],n2, root->mylevel+1);
			root->pointers[i] = (void*)t;
...
int main() {
	...
	FILE *key_file=fopen("key_file.txt","r");
	if(key_file==NULL){
		FILE *key_file = fopen("key_file.txt", "w");
		fprintf(key_file, "%d", 0);
	}
	int key,c,i=0,m=0;
	fscanf(key_file,"%d",&c);
	fclose(key_file);
	...
	while (scanf("%c", &instruction) != EOF) {
		...
		case 'd':
			...
			if(i!=0){
				strcpy(command, "rm *.isl");
				system(command);
				strcpy(command, "rm *.key");
				system(command);
				strcpy(command, "rm *.ml");
				system(command);
				strcpy(command, "rm *.nk");
				system(command);
				strcpy(command, "rm *.point");
				system(command);
				strcpy(command, "rm *.par");
				system(command);
